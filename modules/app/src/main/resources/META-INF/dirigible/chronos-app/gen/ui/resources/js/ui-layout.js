	...
	this.init = function(containerEl, viewNames, id, reconstruct){
		...
		if(id){
			if(!reconstruct){
				...
				var savedState = localStorage.getItem('DIRIGIBLE.application.chronos-app.'+ id);
				if(savedState !== null) {
		...
		let focusView = localStorage.getItem("DIRIGIBLE.application.chronos-app.focusView");
		...
		if(id){
			this.layout.on('stateChanged', function(){
				...
				localStorage.setItem('DIRIGIBLE.application.chronos-app.'+ id, state );
			}.bind(this));			
		}
