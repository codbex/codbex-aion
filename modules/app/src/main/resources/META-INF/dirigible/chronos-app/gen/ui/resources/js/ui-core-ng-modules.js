...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/chronos-app/gen/shell/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/chronos-app/gen/shell/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/chronos-app/gen/shell/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v4/js/chronos-app/gen/shell/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/chronos-app/gen/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.chronos-app.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.chronos-app.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v4/web/chronos-app/gen/ui/resources/templates/sidebar.html'
	}
}])
	...
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.setActiveView = function(activeView) {
				localStorage.setItem('DIRIGIBLE.application.chronos-app.activePerspective', activeView.group);
				localStorage.setItem('DIRIGIBLE.application.chronos-app.focusView', activeView.name);
			};
		},
		templateUrl: relativePath + 'services/v4/web/chronos-app/gen/ui/resources/templates/tiles.html'
	}
}])
