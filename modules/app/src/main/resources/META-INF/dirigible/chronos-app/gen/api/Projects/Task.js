...
const dao = require("chronos-app/gen/dao/Projects/Task");
...
rs.service()
	...
	.resource("count/{ProjectId}")
		.get(function(ctx, request) {
			let ProjectId = parseInt(ctx.pathParameters.ProjectId);
			ProjectId = isNaN(ProjectId) ? ctx.pathParameters.ProjectId : ProjectId;
			http.sendResponseOk("" + dao.count(ProjectId));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("Task not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/v4/js/chronos-app/gen/api/Task.js/" + entity.Id);
